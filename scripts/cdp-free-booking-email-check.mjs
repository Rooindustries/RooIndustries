...
const BOOKING_DISCORD = String(
  process.env.FREE_BOOKING_DISCORD ||
    `servi-${RUN_ID.replace(/[^a-z0-9]/gi, "").slice(-10) || "smoke"}`
).trim();
...
const installPageInstrumentation = async (page) => {
  await page.addInitScript((nextRunId) => {
    window.__serviRunId = nextRunId;
    window.__serviFetchLog = [];
    window.__serviPageErrors = [];
    const originalFetch = window.fetch.bind(window);

    window.addEventListener("error", (event) => {
      window.__serviPageErrors.push({
        type: "error",
  ...
  while (Date.now() - startedAt < timeout) {
    matched = await page.evaluate(
      ({ nextUrlIncludes, nextMethod, nextStatus }) => {
        const entries = Array.isArray(window.__serviFetchLog)
          ? window.__serviFetchLog
          : [];
        return (
          entries.find((entry) => {
...
const getPageErrors = async (page) =>
  page.evaluate(() =>
    Array.isArray(window.__serviPageErrors) ? window.__serviPageErrors : []
  );
...
const runBookingFlow = async ({ page, packageDoc, slot, couponCode }) => {
  ...
  await bookingDialog
    ...
    .fill(`Servi free-booking smoke ${RUN_ID}`);
  await bookingDialog
