...
alter table accounts.creator_profiles
  add constraint creator_profiles_total_basis_points_check
    check (total_basis_points between 0 and 10000),
  add constraint creator_profiles_terms_allocation_check
    check (commission_basis_points + discount_basis_points <= total_basis_points),
...
revoke all on function public.roo_admin_list_creator_terms(text, integer)
...
grant execute on function public.roo_admin_list_creator_terms(text, integer)
  to service_role;
