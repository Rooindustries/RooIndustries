...
create or replace function migration.finalize_inactive_noop_discord_assignments(
...
begin
  ...
  with candidates as (
    select assignment.principal_id, assignment.user_id,
      assignment.discord_user_id, assignment.generation
    ...
    join accounts.tourney_accounts account
      ...
      and account.principal_id = assignment.principal_id
    where assignment.status in ('pending', 'retry', 'processing')
      ...
      and (not account.active or account.lifecycle_status <> 'approved')
      and exists (
        select 1
  ...
  then
    raise exception 'No-op Discord repair requires paused hardened Supabase generation 1'
      using errcode = '55000';
  end if;

  v_command_id := 'discord-noop-finalize:' || v_candidate_digest;
  ...
  insert into tourney.command_receipts (
    ...
    v_command_id,
    'discord:inactive-noop-finalize',
    encode(extensions.digest(v_command_id, 'sha256'), 'hex'),
    'completed',
    200,
    jsonb_build_object(
      'ok', true,
      'finalized', v_candidate_count,
      'reason', 'inactive_desired_state_already_applied'
    ),
    1,
    now(),
    now(),
  ...
  set status = 'applied',
    ...
    applied_at = coalesce(assignment.applied_at, now()),
    lease_id = null,
  ...
  where assignment.status in ('pending', 'retry', 'processing')
    ...
    and exists (
      ...
      where account.user_id = assignment.user_id
        and account.principal_id = assignment.principal_id
        and (not account.active or account.lifecycle_status <> 'approved')
    )
    and exists (
  ...
  if v_updated_count <> v_candidate_count then
    raise exception 'No-op Discord repair candidate set changed'
      using errcode = '40001';
  end if;
...
revoke all on function migration.finalize_inactive_noop_discord_assignments(boolean)
