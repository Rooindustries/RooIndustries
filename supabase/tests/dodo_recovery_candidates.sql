...
insert into migration.source_documents(legacy_sanity_id,payload)
select legacy_sanity_id,jsonb_build_object('_id',legacy_sanity_id,'providerRecoveryTerminal',true,'requiresReschedule',id='reschedule-sync') from commerce.payment_records where id in ('refund-sync','release-sync','email-sync','reschedule-sync');
...
if jsonb_array_length(result)<>5 or not result @> '[{"_id":"paypal-eligible"},{"_id":"refund-sync"},{"_id":"release-sync"},{"_id":"email-sync"},{"_id":"reschedule-sync"}]' then raise exception 'Local recovery was lost: %',result; end if;
